...
function sfile_check_php() {
	$php = '8.1.0'; // @see: http://php.net/manual/de/function.password-hash.php .mcrypt_create_iv( 22, MCRYPT_DEV_URANDOM ).
...
function sfile_check_wp() {
	...
	if ( version_compare( $wp_version, $wp, '<' ) ) {
		return sprintf(
			...
			esc_html__( 'It requires a WordPress version greater than %1$s.', 'sfile' ),
			$wp
		);
	}
}
...
function sfile_check_uploads() {
	if ( defined( 'UPLOADS' ) ) {
		return esc_html__(
			'Having the constant <code>UPLOADS</code> defined is currently not supported. You might have an old db where "ms_files_rewriting" is set "1" or not present (see ms_upload_constants). Things might break.',
			'sfile'
		);
	}
}
...
function sfile_check_config() {
	...
	if ( ! defined( 'FILE_SALT' ) ) {
		$msg = esc_html__( 'Make sure the FILE_SALT constant is defined.', 'sfile' );
	}
	...
	if ( ! count( sfile_search_in_file( $config_file, $search ) ) ) {
		$msg .= esc_html__( 'Please add the following code to your wp-config file:', 'sfile' );
...
function sfile_check_htaccess() {
	...
	if ( ! count( sfile_search_in_file( $htaccess_file, $search ) ) ) {
		$msg  = __( 'Please add the following code to your .htaccess file:', 'sfile' );
 ...
 * @param  [string] $search_for for The string to search for.
 * @return [array] The matches.
 */
function sfile_search_in_file( $file, $search_for ) {
